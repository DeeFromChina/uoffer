...
function checkpsw(id,name,len1,len,isEmpty){
	if(!isEmpty){
		if(document.getElementById(id).value == ''){
			alert("请输入"+name);
			return false;
	...
	if(document.getElementById(id).value.length > len || document.getElementById(id).value.length < len1){
		alert(name+"字符长度要在4~16之间");
		return false;
	}
	return true;
}
