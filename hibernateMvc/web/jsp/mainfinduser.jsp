...
<script language="javascript">
	...
	function init() {
		...
		addbutton("l",data.lastbutton,"lastbutton");
	...
	function selectDownValue(ke, val, targetInput){
		...
		if(targetInput == 'saved'){
			...
			var url = "firm/findSave.do";
			var formId = "finduserForm";
			var data = ajaxSumbit(url, formId);
			if (data != 'error') {
				document.getElementById('lastChoose').value="";
	...
	function addbutton(name,items,buttonId){
		...
		for(var i in items){
			inn = inn + "<button type='button' class='close' aria-hidden='true' style='float:left;opacity:0.9;background:#00B38A;color:#FFF;padding:10px;'>"
				+"<font style='font-size:14px;color:#FFF;font-weight: normal;letter-spacing:2px;'>" + items[i].value 
	...
	function removebutton(obj){
		...
		//document.getElementById('firstIn').value = 2;
		document.getElementById('removeWho').value=value;
		var url = "firm/findRemoveOne.do";//删除一个
		var formId = "finduserForm";
		var data = ajaxSumbit(url, formId);
		if (data == 'error') {
			alert("数据访问出错！");
	...
	function buttonClean(){
		...
		document.getElementById('lastChoose').value="";
		document.getElementById('firmsaved').value="";
		document.getElementById('firmsavedValue').value="";
		document.getElementById('firmcity').value="";
		document.getElementById('firmcityValue').value="";
		document.getElementById('firmjob').value="";
		document.getElementById('firmjobValue').value="";
	...
	function addallorpair(allnum,pairnum){
		document.getElementById('divall').innerHTML="<font style='font-size:14px;color:#4F4F4F;'>全部（"+allnum+")</font>";
		document.getElementById('divpair').innerHTML="<font style='font-size:14px;color:#4F4F4F;'>已收藏（"+pairnum+")</font>";
	}
	function goView(str){
		if(str == 'all'){
	...
	function goSumbit(){
		...
		if (document.getElementById('lastChoose').value != "") {
			flag = true;
		}
		if (flag) {
			document.getElementById('firstIn').value = 2;
...
<body bgcolor="#FFF" onload="init()">
	...
	<br />
	<br />
	<table width="100%" height="94%" id="t1" style="min-width:1000px;display:none;">
		<tr height="30px"/>
		<tr>
			<td width="16%"/>
			<th colspan="2">
		...
		<tr>
			<td/>
			<td style="background:#F4F9F9;width:55%;">
				...
				<div id="div1" style="padding:10px;width:100%;">
					<table width="100%">
						...
						<tr>
							...
							<td width="18%" style="max-width:180px;">
								<div class="input-group" style="max-width: 180px;min-width:140px;">
									<div class="dropdown">
										...
										<input type="text" name="firmsavedValue" id="firmsavedValue" class="form-control" style="width:70%;max-width: 120px;min-width:80px;" placeholder="已保存的搜索" readonly>
									    ...
									    <ul class="dropdown-menu" role="menu" aria-labelledby="dropdownMenu1" style="width:50%;" id="saved"><!--  20px  -->
									    </ul>
									</div>
								</div>
							</td>
							<td width="18%" style="max-width:180px;">
								<div class="input-group" style="max-width: 180px;min-width:140px;">
									<div class="dropdown" style="">
										...
										<input type="text" name="firmcityValue" id="firmcityValue" class="form-control" style="width:70%;max-width: 120px;min-width:80px;" placeholder="城市" readonly>
									    <button type="button" style="width:41px;height:41px !important;border:1px #CCCCCC solid;background-color:#F5F8F9;" class="btn dropdown-toggle" id="dropdownMenu1" data-toggle="dropdown">
									        <span class="caret"></span>
									    </button>
									    <ul class="dropdown-menu" role="menu" aria-labelledby="dropdownMenu1" style="width:20%;" id="city">
									    </ul>
									</div>
								</div>
							</td>
							<td width="18%" style="max-width:180px;">
								<div class="input-group" style="max-width: 180px;min-width:140px;">
									<div class="dropdown" style="">
										...
										<input type="text" name="firmjobValue" id="firmjobValue" class="form-control" style="width:70%;max-width: 120px;min-width:80px;" placeholder="职位" readonly>
									    <button type="button" style="width:41px;height:41px !important;border:1px #CCCCCC solid;background-color:#F5F8F9;" class="btn dropdown-toggle" id="dropdownMenu1" data-toggle="dropdown">
									        <span class="caret"></span>
									    </button>
									    <ul class="dropdown-menu" role="menu" aria-labelledby="dropdownMenu1" style="width:50%;" id="job">
							...
							<td width="18%" style="max-width:180px;">
								<div class="input-group" style="max-width: 180px;min-width:140px;">
									<div class="dropdown" style="">
										...
										<input type="text" name="firmworkedValue" id="firmworkedValue" class="form-control" style="width:70%;max-width: 120px;min-width:80px;" placeholder="工作经验" readonly>
									    <button type="button" style="width:41px;height:41px !important;border:1px #CCCCCC solid;background-color:#F5F8F9;" class="btn dropdown-toggle" id="dropdownMenu1" data-toggle="dropdown">
									        <span class="caret"></span>
									    </button>
									    <ul class="dropdown-menu" role="menu" aria-labelledby="dropdownMenu1" style="width:50%;" id="worked">
									    </ul>
									</div>
								</div>
							</td>
							<td style="padding-right:28px;">
								<input type="text" id="lastChoose" name="lastChoose" class="form-control" style="min-width: 200px;width:100%;" placeholder="">
							</td>
						</tr>
						<tr height="50px"/>
						<tr height="30px">
							...
							<td colspan="3">
								<table>
									<tr>
										...
										<td class="textL" style="padding-left:10px;padding-right:10px;" id="lastbutton">
										</td>
									</tr>
								</table>
							</td>
							<td class="textR" colspan="2" style="padding-right:28px;">
			...
			<td  width="14%">
				<div style="background: #FFFFFF; width: 200px; margin-top:10px;padding-left:20px;">
					<table width="100%">
						<tr>
							<td valign="bottom" width="20px"><font style='font-size:14px;color:#4F4F4F;'>家骏</font></td>
						...
						<tr height="7px">
							<td><font style='font-size:14px;color:#4F4F4F;'>客户服务</font></td>
						</tr>
						<tr>
							<td colspan="2">
		...
		<tr height="20px">
			<td/>
			<td>
				<font style="font-size:14px;">共<font color="#00B38A"><span id="dataNum">10</span></font>个匹配项，显示1~20</font>
			</td>
			<td>
				<input type="hidden" id="allorpair" name="allorpair"/>
				<table width="100%">
					<tr>
						<td class="textC"><div style="cursor:pointer;width:80px;" onclick="goView('all')" id="divall"><font style="font-size:14px;">全部(100)</font></div></td>
						<td class="textC"><div style="cursor:pointer;width:80px;" onclick="goView('pair')" id="divpair"><font style="font-size:14px;">已收藏(100)</font></div></td>
					</tr>
				</table>
			</td>
			<td/>
		...
		<tr>
			...
			<td style="background:#DDDDDD;height:1px;">
				<table width="100%">
					<tr>
						<td width="50%" class="textC"><div style="width:90%;background:#00B38A;height:2px;" id="all"></div><td>
						<td width="50%" class="textC"><div style="width:90%;background:#DDDDDD;height:2px;" id="pair"></div><td>
					</tr>
				</table>
			</td>
			<td/>
		...
		<tr>
			<td/>
			<td colspan="2">
				<iframe id="iframe1" name="iframe1" width="100%" onload="changeFrameHeight()" style="" scrolling="no" frameborder="0" marginheight="0px" marginwidth="0px" frameborder="0" ></iframe>
			</td>
			<td/>
		</tr>
		<tr id="pagecontrol">
				...
				<div id="div1" style="padding:10px;width:100%;">
					<table width="100%">
						<tr>
							...
							<td>
								<input type="text" name="lastChoose" class="form-control" style="width: 280px !important" placeholder="">
							</td>
						</tr>
						<tr height="20px"/>
						<tr height="30px">
