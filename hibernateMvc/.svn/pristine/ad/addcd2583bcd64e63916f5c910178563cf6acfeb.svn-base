			...
			for(var i in items){
				...
				if (isNowAdmin == 1) {
					if (isAdmin != 1) {
						...
						if (teamPeopleStatus == 1) {
							me += "<a href='#' color='00B38A' id='"+items[i].id+"' onclick='goChangeStatus(0,"+items[i].id+")'>禁用</a>";
						}
						else {
							me += "<a href='#' color='00B38A' id='"+items[i].id+"' onclick='goChangeStatus(1,"+items[i].id+")'>启用</a>";
						}
