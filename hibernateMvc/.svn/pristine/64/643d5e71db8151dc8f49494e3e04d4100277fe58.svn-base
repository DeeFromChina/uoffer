...
<script language="javascript">
	...
	function goSearch(){
		...
		if(data.firmList != undefined && data.firmList != ''){
			parent.window.iframe2.location.href="user_qiuzhi_object_view.jsp";
		}
	}
		...
		for(var i in items){
			div1 = div1+"<div class='getColor' onclick='goAllview(this.id)' onmouseover='changeColor3(this)' onmouseout='changeColor2(this)' style='width:100%;padding-top:15px;padding-left:20px;' id='"+items[i].id+"'>"
						+"<table style='width: 100%;'>"
							...
							+"</tr>"
							+"<tr>"
								+"<td class='textL' style='padding-left:20px;'><font size='1'>" +items[i].address+"</font></td>"
							+"</tr>"
							+"<tr>"
								+"<td class='textL' style='padding-left:20px;'><font size='1'>"+items[i].job+"</font></td>"
							+"</tr>"
							+"<tr>"
								+"<td class='textL' style='padding-left:20px;'><font size='1'>"+items[i].money+"</font></td>"
							+"</tr>"
							+"<tr height='20px'>"
								+"<td class='textC' colspan='2'><font size='1'>"+items[i].time+"</font></td>"
							+"</tr>"
						+"</table>"
					+"</div>";
		}
