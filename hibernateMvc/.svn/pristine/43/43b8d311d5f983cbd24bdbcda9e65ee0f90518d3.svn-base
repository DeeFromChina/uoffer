...
//         }
   ...
   public OfferFirmChoose newSaveOfferFirmChoose(Map<String, String> map) throws Exception {//保存选筛选条件，5条记录
      String firmcity = map.get("firmcity");
      String firmjobValue = map.get("firmjob");//firmjobValue
      String firmworkedValue = map.get("firmworkedValue");
      String lastChoose = map.get("lastChoose");
	   ...
	   try {
         ...
         if (offerFirms.size() == 1) {
            ...
            if (offerFirmInformations.size() == 1) {
               ...
               String firmxuqiu = map.get("firmxuqiuValue");
               ...
               offerFirmInformation.setFirmxuqiu(firmxuqiu);
...
//	   }
	   ...
	   str = returnString(nowMap.get("firmjob"));
      if (!"".equals(str) && !str.equals("-1")) {
         param.append(" AND (ou.offer_job_id = '" + str + "' or oui.offer_user_job1 = '" + str + "' or oui.offer_user_job2 = '" + str + "') ");
      }
      str = returnString(nowMap.get("firmstatusValue"));
      if (!"".equals(str) && !str.equals("流程状态")) {
         param.append(" AND (cs1.code_name = '" + str + "' OR cs2.code_name = '" + str + "')");
      }
...
//	   List<Map<String, String>> list = baseDao.findBySql(param.toString());
	...
	@Override
	public List<Map<String, String>> getfrimfinduser(String cityIds,String jobIds, String workTime,String endless) throws Exception {
      ...
      StringBuffer sql1 = new StringBuffer();
		if (!"".equals(workTime)) {
         if (workTime.equals("3年以下")) {
            sql1.append(" oui.OFFER_USER_WORKTIME ='" + workTime + "'");
         }
         else if (workTime.equals("3-5年")) {
            sql1.append(" oui.OFFER_USER_WORKTIME IN('3年','4年','5年')");
         }
         else if (workTime.equals("5-8年")) {
            sql1.append(" oui.OFFER_USER_WORKTIME IN('5年','6年','7年','8年')");
         }
         else if (workTime.equals("8-10年")) {
            sql1.append(" oui.OFFER_USER_WORKTIME IN('8年','9年','10年')");
         }
         else if (workTime.equals("10年以上")) {
            sql1.append(" oui.OFFER_USER_WORKTIME ='" + workTime + "'");
         }
         else{
            sql1.append(" oui.OFFER_USER_WORKTIME IN('" + workTime + "')");
         }
         sql1.append(" and ");
       }
		param.append( " and !find_in_set(SUBSTRING('"+endless+"',  INSTR('"+endless+"', '@')), ifnull(oui.endless,''))  AND "+sql1+" ( oui.offer_user_job1 in ("+jobIds+
   ...
   @Override
   public List<Map<String, String>> getfrimfinduserfortwo(String userId,String cityIds, String jobIds, String workTime,String endless) throws Exception {
      // TODO Auto-generated method stub
      StringBuffer param = new StringBuffer();
      Map<String, String> map = new HashMap<String, String>();
      StringBuffer sql1 = new StringBuffer();
      ...
      param.append( " and !find_in_set(SUBSTRING('"+endless+"',  INSTR('"+endless+"', '@')), ifnull(oui.endless,'')) AND "+sql1+" (oui.offer_user_job1 in ("+jobIds+
                 ") OR oui.offer_user_job2 in ("+jobIds+
                 ") OR oc.id in ("+cityIds+"))"+" AND ou.id not in (SELECT om.user_id FROM offer_match om WHERE om.firm_user_id = '"+userId+"')");
      map.put("param", param.toString());
      List<Map<String, String>> list = baseDao.findByQuery("/sql/sql_finduser_tuijian.xml", map);
      return list;
   }
